-- Complete navigation setup with constraint fixes
-- Run this entire script in Supabase SQL Editor

-- First, add the required unique constraints (ignore errors if they already exist)
DO $$
BEGIN
  -- Add unique constraint on s42_categories.name
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 's42_categories_name_key'
  ) THEN
    ALTER TABLE public.s42_categories
    ADD CONSTRAINT s42_categories_name_key UNIQUE (name);
  END IF;

  -- Add unique constraint on s42_menu_items (category_id, href)
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 's42_menu_items_category_href_key'
  ) THEN
...
SELECT 'Categories:' as info, COUNT(*) as count FROM public.s42_categories
UNION ALL
SELECT 'Pages:', COUNT(*) FROM public.s42_pages
UNION ALL
SELECT 'Menu Items:', COUNT(*) FROM public.s42_menu_items;